...
async function main() {
  ...
  api = http.createServer(async (req, res) => {
    ...
    if (method === 'getSession') result = { authenticated: true, user: { id: account, username: account } };
    ...
    else if (method === 'searchMedia' || method === 'getDiscoverMedia') result = await metadataService.query(method, args);
...
main().then(() => {
  fs.writeFileSync(resultPath, JSON.stringify({ status: 'passed', checks: ['Chromium renderer', 'IndexedDB queue and reload', 'favorites', 'account isolation', 'entry editor', 'full renderer API', 'cloud backup', 'preference aliases', 'AniList import selection and review', 'import cancellation', 'real cache service provider-outage flow', 'saved-catalog fallback', 'retry backoff', 'MAL fallback and AniList recovery', 'MAL-only manga details', 'MAL import preview, cancellation, selection and private review'] }));
  app.exit(0);
}).catch(error => {
